...
class Flash():
    ...
    def __init__(self, driver, stlink, dbg, bank=0):
        ...
        if self._stlink.target_voltage < 2.0:
            raise stlib.stlinkex.StlinkException('Supply voltage is %.2fV, but minimum for FLASH program or erase is 2.0V' % self._stlink.target_voltage)
        self.unlock()
    ...
    def unlock(self):
        ...
        # programing locked
        if self._stlink.get_debugreg32(Flash.FLASH_CR_REG) & Flash.FLASH_CR_LOCK_BIT:
            raise stlib.stlinkex.StlinkException('Error unlocking FLASH')
    ...
    def wait_busy(self, wait_time, bargraph_msg=None):
        ...
        raise stlib.stlinkex.StlinkException('Operation timeout')

    def wait_for_breakpoint(self, wait_time):
        end_time = time.time() + wait_time
        while time.time() < end_time and not self._stlink.get_debugreg32(stlib.stm32.Stm32.DHCSR_REG) & stlib.stm32.Stm32.DHCSR_STATUS_HALT_BIT:
            time.sleep(wait_time / 20)
    ...
    def end_of_operation(self, status):
        if status != Flash.FLASH_SR_EOP_BIT:
            raise stlib.stlinkex.StlinkException('Error writing FLASH with status (FLASH_SR) %08x' % status)
...
class Stm32FP(stlib.stm32.Stm32):
    ...
    def _flash_write(self, addr, data, erase=False, verify=False, erase_sizes=None, bank=0):
        ...
        while(data):
            ...
            if verify and block != self._stlink.get_mem32(addr, len(block)):
                raise stlib.stlinkex.StlinkException('Verify error at block address: 0x%08x' % addr)
            addr += len(block)
        flash.lock()
        self._dbg.bargraph_done()
    ...
    def flash_write(self, addr, data, erase=False, verify=False, erase_sizes=None):
        ...
        if addr is None:
            ...
            raise stlib.stlinkex.StlinkException('Start address is not aligned to half-word')
