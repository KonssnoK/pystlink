    ...
    def get_voltage_dependend_params(self):
        ...
        for params in Flash.VOLTAGE_DEPENDEND_PARAMS:
            if self._stlink.target_voltage > params['min_voltage']:
                return params
        raise stlib.stlinkex.StlinkException('Supply voltage is %.2fV, but minimum for FLASH program or erase is 1.8V' % self._stlink.target_voltage)
...
# (STM32F2xx, STM32F4xx)
class Stm32FS(stlib.stm32.Stm32):
    ...
    def flash_write(self, addr, data, erase=False, verify=False, erase_sizes=None):
        ...
        if addr % 4:
            raise stlib.stlinkex.StlinkException('Start address is not aligned to word')
        # align data
        if len(data) % 4:
